...
import { getMerchants } from './merchantStorage';
 ...
 *
 * Transaction names are typed by whoever created the payment, so they are
 * matched loosely — the same rule the XP engine uses to find a merchant's
 * earning rate, kept in one place so a sale that earns XP is always the same
 * sale that shows up in that merchant's insights.
 */
export function matchesMerchant(transactionName: string, merchantName: string): boolean {
  const name = transactionName.trim().toLowerCase();
...
export function computeMerchantInsight(
  ...
  const purchases = allPurchases.filter(p => matchesMerchant(p.name, merchantName));
  const redemptions = allRedemptions.filter(r => matchesMerchant(r.merchant, merchantName));
...
export function getMerchantInsight(merchantName: string): MerchantInsight {
  return computeMerchantInsight(getPurchases(), getAllRedemptions(), merchantName);
}
