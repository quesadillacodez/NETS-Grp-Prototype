...
// The fixture names sales "Kopitiam Toa Payoh"; under exact-or-alias matching
// that variant is declared rather than inferred from a substring.
const KOPITIAM = ['Kopitiam Toa Payoh'];

describe('matching a sale to a merchant', () => {
  it('matches the merchant name exactly', () => {
    expect(matchesMerchant('Kopitiam', 'Kopitiam')).toBe(true);
  });

  it('matches a declared alias', () => {
    // Variants are declared rather than guessed, so the merchant decides which
    // transaction names belong to it.
    expect(matchesMerchant('Kopitiam Toa Payoh', 'Kopitiam', ['Kopitiam Toa Payoh'])).toBe(true);
  });

  it('ignores case and padding', () => {
    expect(matchesMerchant('  FAIRPRICE ', 'FairPrice')).toBe(true);
    expect(matchesMerchant('  FAIRPRICE Finest ', 'FairPrice', ['FairPrice Finest'])).toBe(true);
  });

  it('no longer swallows a longer name that merely contains the merchant', () => {
    // A merchant called "Kopi" used to claim every "Kopitiam Food Court" sale.
    expect(matchesMerchant('Kopitiam Food Court', 'Kopi')).toBe(false);
    expect(matchesMerchant('FAIRPRICE Finest', 'FairPrice')).toBe(false);
  });
...
describe('what a merchant is told about its sales', () => {
  ...
  const insight = computeMerchantInsight(purchases, [], 'Kopitiam', KOPITIAM);
